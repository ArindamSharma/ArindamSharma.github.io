...
const ExperienceSection = () => {
  ...
  const goToExperience = useCallback((index) => {
    setCurrentExperience(index);
  }, []);

  // Get card position class for 3D stacking effect
  const getCardClass = (index) => {
    const diff = index - currentExperience;
    ...
    if (normalizedDiff === 1 || normalizedDiff === -totalCards + 1) return 'next';
    if (normalizedDiff === -1 || normalizedDiff === totalCards - 1) return 'prev';
    if (normalizedDiff > 1 || normalizedDiff < -totalCards + 1) return 'far-next';
    if (normalizedDiff < -1 || normalizedDiff > totalCards - 1) return 'far-prev';
  ...
  return (
    <section className="experience-section section" id="experience">
      <div className="container">
        ...
        >
          <div className="experience-3d-carousel">
            {experiences.map((exp, index) => (
              <div 
                key={index}
                className={`experience-card-3d ${getCardClass(index)}`}
